...
num_cores = 10
cardinalities = np.unique(np.linspace(5, 25, 21, dtype=int))

function_arguments = {}
function_arguments['N'] = 1000
function_arguments['m'] = 1000
function_arguments['delta_n'] = 0
Nexp = 10

# unbiased random walk with higher-order step
# check that the directories exist
save_to = 'data/uniform/exp_urw_ho'
processed_list = Parallel(n_jobs=num_cores)(delayed(HRW.rw_on_generated_hypergraphs)(HM.Generate_Uniform_Hypergraph,
                                                                                     update_dict(function_arguments,'cardinality',i),
                                                                                     Nexp,
                                                                                     'urw',
                                                                                     'ho',
                                                                                     save_to + f'_cardinality_{i:.2f}.txt') for i in tqdm(cardinalities))

# unbiased random walk with projected step
save_to = 'data/uniform/exp_urw_p'
processed_list = Parallel(n_jobs=num_cores)(delayed(HRW.rw_on_generated_hypergraphs)(HM.Generate_Uniform_Hypergraph,
                                                                                     update_dict(function_arguments,'cardinality',i),
                                                                                     Nexp,
                                                                                     'urw',
...
save_to = 'data/uniform/exp_merw_ho'
processed_list = Parallel(n_jobs=num_cores)(delayed(HRW.rw_on_generated_hypergraphs)(HM.Generate_Uniform_Hypergraph,
                                                                                     update_dict(function_arguments,'cardinality',i),
                                                                                     Nexp,
                                                                                     'merw',
...
save_to = 'data/uniform/exp_merw_p'
processed_list = Parallel(n_jobs=num_cores)(delayed(HRW.rw_on_generated_hypergraphs)(HM.Generate_Uniform_Hypergraph,
                                                                                     update_dict(function_arguments,'cardinality',i),
                                                                                     Nexp,
                                                                                     'merw',
